...
analyse_pas <- function(data, input_file) {
  ...
  iplc_area <- tibble(
    var = "iplc_area",
    kfw = sum(pa_data$iplc[pa_data$wh == 1 & is_kfw]),
    giz = sum(pa_data$iplc[pa_data$wh == 1 & is_giz]),
    both = sum(pa_data$iplc[pa_data$wh == 1 & is_both]),
    total =sum(pa_data$iplc[pa_data$wh == 1])
  )
